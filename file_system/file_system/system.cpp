 ...
 */
 ...
 ///***** 定义的常量 *****/
#define BLOCKSIZE       1024        // 磁盘块大小，每个块1024字节=1KB
#define SIZE            1024000     // 虚拟磁盘空间大小，共划分成1000个磁盘块（引导块占1个盘块，FAT1,FAT2各占据2个盘块，其他盘块均为数据区）
...
typedef struct BootBlock {
    char magic_number[8];       // 文件系统的数
    char information[200];      // 描述信息，比如磁盘块大小、磁盘块数量
...
useropen openfilelist[MAXOPENFILE]; // 用户打开文件表数组，最多同时打开10个文件
...
int  find_father_dir(int fd);                        // 寻找一个打开文件的父目录
...
void set_fileopenlist(int fd, fcb* fcbPtr)  //将fcbptr的内容赋值到文件打开表的第fd项中
{
...
void startsys() {
 ...
 /*   strcpy(openfilelist[0].filename, root->filename);     //改进
...
void my_cd(char* dirname) {
    ...
    //如果是目录文件
        ...
        if (i == int(openfilelist[currfd].length / sizeof(fcb)))    //改进
        {
            cout << "不存在这个目录文件！" << endl;
            return;
        }
        ...
        //如果cd 了一个目录文件, 那么判断是.还是..还是子文件,如果是子文件则打开这个目录文件到openfilelist里
            ...
            //cd ..需要判断现在是不是根目录, 如果是根目录,不操作, 否则,返回上一层
                ...
                if (currfd == 0) {
                    ...
                    currfd = my_close(currfd);  //my_close正常返回当前fd的上一级fd
                    return;
                }
            }
            //cd 当前目录下的子文件
            else {
                int fd = get_Free_Openfile();   //找到空的用户打开表项
                if (fd == -1) { //没有空的表项了
                    return;
                }
                else {
...
void my_mkdir(char* dirname) {
    ...
    char text[MAX_TEXT_SIZE];   //允许读入的最大文件大小
    openfilelist[currfd].filePtr = 0;   //用户打开文件表数组，currfd记录当前目录的文件描述符
    int fileLen = do_read(currfd, openfilelist[currfd].length, text);//要求读取的长度为length，返回值为实际读取的长度。要读的文件长度存入到text中
    //text里的内容就是一个个fcb
    fcb* fcbPtr = (fcb*)text;
    ...
    fcbtmp->length = 2 * sizeof(fcb);   //新建的目录文件里面有两个特殊目录项.和..
    fcbtmp->free = 1;
 ...
 //同理这里也不能直接写进fcbPtr里,只能用fcbTmp写进内存里,然后用do_write转写进文件
    ...
    do_write(fd, (char*)fcbtmp2, sizeof(fcb), 1);   //进行写操作时,fd的length就进行相应加改变了
    ...
    my_close(fd);
    //更新本currfd目录文件的fcb
    fcbPtr->length = openfilelist[currfd].length;   //上面在写currfd的时候就已经把它的长度改变了
    openfilelist[currfd].filePtr = 0;
...
void my_rmdir(char* dirname) {
    ...
    openfilelist[currfd].filePtr = 0;
    do_read(currfd, openfilelist[currfd].length, buf);// 根据currfd找到起始盘块号，要读的文件存入到buf中
    int i;
    fcb* fcbPtr = (fcb*)buf;
    //寻找叫这个名字的文件目录项
    for (i = 0; i < int(openfilelist[currfd].length / sizeof(fcb)); i++, fcbPtr++) {
        if (strcmp(fcbPtr->filename, fname) == 0 && fcbPtr->metadata == 0) {    //metddata=0表示是目录文件
            break;
        }
    }
    if (i == int(openfilelist[currfd].length / sizeof(fcb))) {  //如果找到了最后一个之后，说明没有这个文件
        cout << "没有这个目录文件" << endl;
        return;
    }
    //判断这个目录文件里,清空了没有,不允许删除没有清空的目录
    if (fcbPtr->length > 2 * sizeof(fcb)) { //除了两个特殊目录文件.和.. 还有其他文件，就不能删除
        cout << "请先清空这个目录下的所有文件,再删除目录文件" << endl;
        return;
    }
    //清空这个目录项占据的FAT
    int blockNum = fcbPtr->first;   //文件起始盘块号
    fat* fat1 = (fat*)(v_addr0 + BLOCKSIZE);    //虚拟磁盘的起始地址+磁盘块大小
    int next = 0;
    while (1) {
        next = fat1[blockNum].id;
        fat1[blockNum].id = END;    //fat.id=free代表未分配，其他代表某文件下一磁盘块的块号
        if (next != END) {
            blockNum = next;    //next一直代表下一磁盘块的块号，一直在沿着表项遍历，end是fat表中的文件结束标志
        }
        else {
            break;
        }
    ...
    fcbPtr->length = 0;
    //写到磁盘上去, 更新fcb内容为空
    openfilelist[currfd].filePtr = i * sizeof(fcb); //fileptr代表读写指针在文件中的位置
    do_write(currfd, (char*)fcbPtr, sizeof(fcb), 1); //把fcptrb内容写入到buf，再把buf内容写入到磁盘块；1:覆盖写:也就是从filePtr指向的内容继续往下写
    openfilelist[currfd].length -= sizeof(fcb);
    //更新.目录项的长度
    fcbPtr->length = openfilelist[currfd].length;
    openfilelist[currfd].filePtr = 0;
    do_write(currfd, (char*)fcbPtr, sizeof(fcb), 1);
    openfilelist[currfd].fcbstate = 1;  //文件fcb修改标志置为1，表示已修改
}
void my_ls() {
    ...
    openfilelist[currfd].filePtr = 0;   //读写指针位置置为起始点
    do_read(currfd, openfilelist[currfd].length, buf);  //将当前目录下的文件读到Buf里
        ...
        if (fcbPtr->free == 1) {
            ...
            if (fcbPtr->metadata == 0) {
                printf("%s\t%dB\t<DIR>\t%d/%d/%d\t%02d:%02d:%02d\n",
                    ...
                    (fcbPtr->date >> 9) + 2000,     //文件创建日期，年，16Bit
...
int  do_write(int fd, char* text, int len, char wstyle) {
    ...
    //2. 如果len>BLCOKSIZE-off, 那么改
    // 
    // 写的内容还没写完, 你要申请盘块, 继续往下写, 一直写到len个字符
    //PS: 在my_write中有3中写法,
...
int  do_read(int fd, int len, char* text) {
    ...
    int blockNum = openfilelist[fd].first;
    //fatPtr 当前盘块对应的fat,根据逻辑块号找到fat表项
    fat* fatPtr = (fat*)(v_addr0 + BLOCKSIZE) + blockNum;
    while (off >= BLOCKSIZE) {
        off -= BLOCKSIZE;
        blockNum = fatPtr->id;
